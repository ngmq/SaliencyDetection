...
std::vector<cv::Mat> colorConspicuityMaps(std::vector<cv::Mat> &lab_channels, int num_layers)
{
...
cv::Mat orientationConspicuityMap(const cv::Mat &m, int num_layers, int num_orientations)
{
  cv::Mat img = m.clone();
